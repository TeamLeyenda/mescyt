...
Return of `backend\admin\components\MenuHelper::getAssignedMenu()` has default format like:
...
return format of `backend\admin\components\MenuHelper::getAssignedMenu()` by provide a callback to this method.
...
Using Sparated Menu
-------------------
Second parameter of `backend\admin\components\MenuHelper::getAssignedMenu()` used to get menu on it's own hierarchy.
...
user backend\admin\components\Helper;
